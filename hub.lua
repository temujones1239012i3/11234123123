...
local function saveSettings()
    ...
    if success then
        print("[Script Hub] Settings saved")
    else
        warn("[Script Hub] Failed to save settings:", err)
    end
end
...
local function loadSettings()
    ...
    if success and result then
        ...
        if decodeSuccess and settings and type(settings) == "table" then
            ...
            print("[Script Hub] Settings loaded")
            return true
        end
    end
    return false
...
ScriptModules["AntiHit"] = {
    ...
    init = function(self)
        ...
        local godmodeConn = self.data.humanoid:GetPropertyChangedSignal("Health"):Connect(function()
            if not self.active then return end
            self.data.humanoid.Health = math.huge
        end)
        table.insert(self.data.connections, godmodeConn)
        ...
        -- Create server position box
        local function createServerPosBox()
            if self.data.serverPosBox then
                self.data.serverPosBox:Destroy()
            end
            
            local box = Instance.new("Part")
            box.Name = "ServerPositionBox"
            ...
            self.data.serverPosBox = box
        end
        
        -- Update server position box
        local function updateServerPosBox()
            if self.data.DESYNC_ENABLED and self.data.humanoidRootPart and self.data.FAKE_POSITION then
                if not self.data.serverPosBox then
                    createServerPosBox()
                end
                self.data.serverPosBox.CFrame = self.data.FAKE_POSITION
                self.data.serverPosBox.Transparency = 0.5
            else
                if self.data.serverPosBox then
                    self.data.serverPosBox.Transparency = 1
                end
            end
        end
        ...
        desyncGui.Name = "DesyncGUI"
        desyncGui.ResetOnSpawn = false
        desyncGui.Parent = player:WaitForChild("PlayerGui")
        ...
        startButton.Text = "DESYNC"
        startButton.TextColor3 = Color3.fromRGB(255, 255, 255)
        startButton.TextSize = 18
        startButton.Font = Enum.Font.GothamBold
        startButton.BorderSizePixel = 0
        startButton.Parent = desyncGui
        ...
        -- Button click (fire event then toggle desync)
        startButton.MouseButton1Click:Connect(function()
            if not self.data.DEBOUNCE then
        ...
        -- F key binding
        local fKeyConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed or not self.active or self.data.DEBOUNCE or input.KeyCode ~= Enum.KeyCode.F then return end
        ...
        table.insert(self.data.connections, fKeyConn)
        
        -- RenderStepped loop
        local renderConn = RunService.RenderStepped:Connect(function()
            if not self.active or not self.data.DESYNC_ENABLED or not self.data.character or not self.data.humanoidRootPart then return end
            
            if self.data.CLIENT_POSITION then
                self.data.humanoidRootPart.CFrame = self.data.CLIENT_POSITION
            end
        ...
        table.insert(self.data.connections, renderConn)
        
        -- Heartbeat loop
        local heartbeatConn = RunService.Heartbeat:Connect(function()
            if not self.active or not self.data.DESYNC_ENABLED or not self.data.character or not self.data.humanoidRootPart or not self.data.FAKE_POSITION then return end
            if tick() - self.data.lastUpdate >= self.data.UPDATE_INTERVAL then
                pcall(function()
                    ...
                    self.data.FAKE_POSITION = self.data.FAKE_POSITION * CFrame.new(moveOffset + randomOffset)
                    
                    for _, part in pairs(self.data.character:GetChildren()) do
                        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                            part.CFrame = self.data.FAKE_POSITION * CFrame.new(
                                math.random(-self.data.OFFSET_RANGE, self.data.OFFSET_RANGE),
                                math.random(-0.5, 0.5),
                                math.random(-self.data.OFFSET_RANGE, self.data.OFFSET_RANGE)
                            )
                        end
        ...
        table.insert(self.data.connections, heartbeatConn)
        
        -- Character respawn
        local respawnConn = player.CharacterAdded:Connect(function(newChar)
            if not self.active then return end
            self.data.character = newChar
            self.data.humanoidRootPart = newChar:WaitForChild("HumanoidRootPart")
            self.data.humanoid = newChar:WaitForChild("Humanoid")
        ...
        self.data.desyncGui = desyncGui
        
        print("[Anti-Hit] Activated - Press F or click button to toggle desync")
    end,
    
    cleanup = function(self)
        if self.data.desyncGui then
            pcall(function() self.data.desyncGui:Destroy() end)
            self.data.desyncGui = nil
        end
        
        if self.data.serverPosBox then
            pcall(function() self.data.serverPosBox:Destroy() end)
            self.data.serverPosBox = nil
        end
        ...
        -- Reset humanoid
        if self.data.humanoid then
            self.data.humanoid.MaxHealth = 100
            self.data.humanoid.Health = 100
        end
...
print("[Script Hub] Loaded successfully! Toggle scripts on/off from the GUI.")